...
# ====================================
# TỰ ĐỘNG TÍNH TOÁN BATCH THEO KHUNG GIỜ RUN
# ====================================
BATCH_SIZE = 10
...
# ====================================
# TIẾN TRÌNH CHẠY CHÍNH (MAIN ENTRY)
# ====================================
if __name__ == '__main__':
    print(f"--- [GITHUB ACTIONS] KÍCH HOẠT QUÉT NHÓM {group_number}/20 ---")
    signals_found = []
    ...
    for symbol in batch_symbols:
        try:
            ...
            # Check bộ lọc điều kiện vùng mua chiến lược
            if (rsi < 30) or (stoch_k < 20 and banker > 15):
                sentiment, news_title = get_news_sentiment(symbol)
                
                if trend_1w == "Uptrend" and banker > 20:
                    verdict = "MUA GOM - Tin xấu ra để đè giá, cá mập âm thầm hấp thụ hết lực bán, cơ hội gom giá tốt." if sentiment == "Tin xấu" else "MUA GOM - Xu hướng lớn ủng hộ, cá mập đang đẩy tiền gom hàng, xác suất nổ tím cao."
                    decision_icon = "🟪"
            ...
            time.sleep(4) # Giãn cách ngắn bảo vệ IP cho tài khoản GitHub runner
            
        except Exception as e:
            print(f"Lỗi tại mã {symbol}: {e}")
            time.sleep(6)
            
    # --- BÁO CÁO DUY NHẤT LÊN TELEGRAM ---
    msg_summary = f"🤖 **[BOT] Chào Bảo, quét lần 1 (Nhóm {group_number}/20): Hoàn thành**\n"
    if len(signals_found) > 0:
        msg_summary += f"🟪 Tìm thấy {len(
